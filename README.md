...
작업폴더 루트(프로젝트 상위)에 책 단위 폴더가 생성된다. 각 단계의 **출력 = 다음 단계 입력**이다:

```
../ocr-output-<날짜>/
  README.md                 # 폴더/파일 규칙
  01/                        # OCR 문제 MD 단계  ← 이 툴(#1)
    01-01.md ~ 07-50.md       # {회차}-{문항}.md
    images/ 01-14_1.png       # {회차}-{문항}_{순번}.png
    _index.json  difficulty_stats.json
  02/ 03/ 04/                 # 집필·요약·영상대본  ← #2 exambook-forge
  05/<회차>/                  # 슬라이드/영상 번들   ← #2(골격)·#3 chodangi-mp4(영상)·리모션
```

- **파이프라인 규약(00~05, 05 번들·review.json·_series)의 단일 진실**은
  exambook-forge `references/pipeline-output-structure.md`. 이 툴은 그 규약의 `01/` 단계를 만든다.
- 회차코드 표기: 원본 기출은 `01-`~`07-`, 자사 재집필 회차는 `mNN`(#2). 05 번들 폴더명 = 회차코드.
